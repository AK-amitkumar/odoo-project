...
class mrp_production_workcenter_line(models.Model):
    ...
    # date_planned_end= fields.Datetime(_get_date_end, string='End Date')
    ...
    production_state=fields.Selection([('draft','Draft'),('confirmed','Waiting Goods'),('ready','Ready to Produce'),('in_production','In Production'),('cancel','Canceled'),('done','Done')]
        ,string='Production Status', readonly=True)
    # production_state=fields.Selection('production_id','state'
    ...
    # product=fields.Many2one('production_id','product_id',relation='product.product',string='Product',
    #     readonly=True)
    qty=fields.Float(string='Qty',readonly=True, store=True)
    uom=fields.Many2one('product.uom',string='Unit of Measure',readonly=True)
...
class mrp_operations_operation(models.Model):
    ...
    #     return True
        ...
        # order_date= fields.Date(_get_order_date,string='Order Date',type='date',store={'mrp.production':(_order_date_search_production,['date_planned'], 10)})
